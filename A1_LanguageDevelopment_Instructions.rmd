...
mean_visit_asd <- 0.12                         #Develops less than TD, also making it relative to the average MLU of ASD. #look at these values again, for TD and ASD.
sigma_visit_asd <- 0.1                        #More heterogeneity, making the sigma to be on the same scale as mean_visit_asd.
...
mean_visit_td <- 0.2                          #Develops more than ASD, , also making it relative to the average MLU of TD.
sigma_visit_td <- 0.06                        #Less heterogeneity, making the sigma to be on the same scale as mean_visit_td.
```
...
pp_check(MLU_mod_prior, ndraws=100) + labs(title = "Prior predictive check") #Prior predictive check. Seems to be in the range of actual data, although looks odd when in log-scale.
...
ggsave("Power_analysis.jpeg", plot = power_plots, path = "/Users/justina/Desktop/Desktop - Justina’s MacBook Pro/Aarhus_Uni/Semester_3/Methods 3/Assignment-1")
...
data_clean$Diagnosis <- as.factor(data_clean$Diagnosis) #Changed all variables that were needed (ID, Gender, Diagnosis...)
